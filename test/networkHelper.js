...
async function listTokenForRedeployNetwork(storageInstance, reserveInstances, tokens, operator) {
  for (const [key, reserve] of Object.entries(reserveInstances)) {
    for (let j = 0; j < tokens.length; j++) {
      await storageInstance.listPairForReserve(reserve.reserveId, tokens[j].address, true, true, true, {
        from: operator,
      });
    }
  }
...
async function setupNetwork(NetworkArtifact, networkProxyAddress, KNCAddress, kyberDaoAddress, admin, operator) {
...
async function addReservesToStorage(storageInstance, reserveInstances, tokens, operator) {
  for (const [key, value] of Object.entries(reserveInstances)) {
    ...
    let rebateWallet =
      reserve.rebateWallet == zeroAddress || reserve.rebateWallet == undefined
        ? reserve.address
        : reserve.rebateWallet;
    await storageInstance.addReserve(reserve.address, reserve.reserveId, reserve.onChainType, rebateWallet, {
      from: operator,
    });
    for (let j = 0; j < tokens.length; j++) {
      await storageInstance.listPairForReserve(reserve.reserveId, tokens[j].address, true, true, true, {
...
module.exports.removeReservesFromStorage = async function (storageInstance, reserveInstances, tokens, operator) {
  for (const [key, value] of Object.entries(reserveInstances)) {
    reserve = value;
    console.log(
      'removing reserve type: ' + reserve.type + ' address: ' + reserve.address + ' pricing: ' + reserve.pricing
    );
    for (let j = 0; j < tokens.length; j++) {
      await storageInstance.listPairForReserve(reserve.reserveId, tokens[j].address, true, true, false, {
        from: operator,
      });
    }
...
async function getWrongHint(rateHelper, reserveInstances, hintType, numReserves, srcAdd, destAdd, qty) {
  function buildHint(tradeType, reserveIds, splits) {
    return web3.eth.abi.encodeParameters(['uint8', 'bytes32[]', 'uint256[]'], [tradeType, reserveIds, splits]);
  }
...
function randomSelectReserves(tradeType, reserves, splits) {
  ...
  if (tradeType == SPLIT_HINTTYPE) {
    ...
    for (i = 0; i < result.reserves.length; i++) {
      ...
      splitValue =
        i == result.reserves.length - 1
          ? BPS.sub(bpsAmtSoFar)
          : new BN(getRandomInt(1, BPS.div(new BN(result.reserves.length))));
...
async function getAndCalcRates(
  matchingEngine,
  storage,
  reserveInstances,
  srcToken,
  ...
  if (srcToken != ethAddress) {
    ...
    for (let i = 0; i < reserves.reserveIds.length; i++) {
      ...
      result.t2eRates[i] = await reserveInstance.getConversionRate(
        srcToken,
        ethAddress,
        result.t2eSrcAmounts[i],
        blockNum
      );
    ...
    for (let i = 0; i < indexes.length; i++) {
      ...
      dstQty = Helper.calcDstQty(
        result.t2eSrcAmounts[indexes[i]],
        srcDecimals,
        ethDecimals,
  ...
  if (destToken != ethAddress) {
    ...
    for (let i = 0; i < reserves.reserveIds.length; i++) {
      ...
      reserveInstance = reserveInstances[reserves.reserveIds[i]].instance;
      result.e2tRates[i] = await reserveInstance.getConversionRate(
        ethAddress,
        destToken,
        result.e2tSrcAmounts[i],
        blockNum
      );
      feeAccountedBps.push(zeroBN);
    }
    ...
    for (let i = 0; i < indexes.length; i++) {
      ...
      dstQty = Helper.calcDstQty(
        result.e2tSrcAmounts[indexes[i]],
        ethDecimals,
        destDecimals,
        result.e2tRates[indexes[i]]
      );
  ...
  destAmountWithNetworkFee = Helper.calcDstQty(
    result.tradeWei.sub(result.networkFeeWei),
    ethDecimals,
    destDecimals,
    e2tRate
  );
...
async function compareBalancesAfterTrade(
  srcToken,
  destToken,
  srcQty,
...
async function calcParamsFromMaxDestAmt(srcToken, destToken, unpackedOutput, info, maxDestAmt) {
  ...
  if (unpackedOutput.actualDestAmount.gt(maxDestAmt)) {
    ...
    // E2T side
    if (destToken != ethAddress) {
      [tradeWeiAfterFees, unpackedOutput.e2tSrcAmounts] = calcTradeSrcAmount(
        ethDecimals,
        await destToken.decimals(),
        maxDestAmt,
        unpackedOutput.e2tRates,
        unpackedOutput.e2tSrcAmounts
      );
    } else {
      tradeWeiAfterFees = maxDestAmt;
    }

    let newTradeWei = tradeWeiAfterFees
      .mul(BPS)
      .mul(BPS)
      .div(BPS.mul(BPS).sub(networkFeeBps.mul(unpackedOutput.feePayingReservesBps)).sub(platformFeeBps.mul(BPS)));
    if (unpackedOutput.tradeWei.gt(newTradeWei)) {
      ...
      console.log(
        'New trade wei is greater than current trade wei by: ' +
          newTradeWei.sub(unpackedOutput.tradeWei).toString(10) +
          ' wei'
      );
    }
    unpackedOutput.networkFeeWei = unpackedOutput.tradeWei
      .mul(networkFeeBps)
      .div(BPS)
      .mul(unpackedOutput.feePayingReservesBps)
      .div(BPS);
    unpackedOutput.platformFeeWei = unpackedOutput.tradeWei.mul(platformFeeBps).div(BPS);

    // T2E side
    if (srcToken != ethAddress) {
      [actualSrcAmt, unpackedOutput.t2eSrcAmounts] = calcTradeSrcAmount(
        await srcToken.decimals(),
        ethDecimals,
        unpackedOutput.tradeWei,
        unpackedOutput.t2eRates,
        unpackedOutput.t2eSrcAmounts
      );
    } else {
...
function calcTradeSrcAmount(srcDecimals, destDecimals, destAmt, rates, srcAmounts) {
  ...
  for (let i = 0; i < srcAmounts.length; i++) {
    let destAmountSplit =
      i == srcAmounts.length - 1
        ? new BN(destAmt).sub(destAmountSoFar)
  ...
  if (destAmt.gt(totalDestAmount)) {
    console.log(
      'MaxDestAmount: new total dest amount is smaller than max dest amount by: ' + destAmt.sub(totalDestAmount)
    );
  }
  if (shouldFallback) {
    srcAmount = new BN(0);
