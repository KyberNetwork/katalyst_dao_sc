...
contract('KyberDao', function (accounts) {
  before('one time init', async () => {
    ...
    kncToken = await Token.new('Kyber Network Crystal', 'KNC', 18);
    victor = accounts[2];
    loi = accounts[3];
    mike = accounts[4];
  ...
  const setupTokenWithSupply = async (supply) => {
    kncToken = await Token.new('test token', 'tst', 18, {from: accounts[0]});
  ...
  describe('#Handle Withdrawal tests', () => {
    ...
    it('Test handle withdrawal updates correct points with multiple voted campaigns - no delegation', async function () {
      ...
      await updateCurrentBlockAndTimestamp();
      let options = [1, 2, 3, 4];
      let txResult = await submitNewCampaign(
        kyberDao,
        0,
        currentBlock + 2,
      ...
      await updateCurrentBlockAndTimestamp();
      txResult = await submitNewCampaign(
        kyberDao,
        1,
        currentBlock + 2,
        currentBlock + 2 + minCampPeriod,
    ...
    it('Test handle withdrawal should revert when sender is not staking', async function () {
      ...
      await expectRevert(kyberDao.handleWithdrawal(victor, 0, {from: victor}), 'only staking contract');
      await expectRevert(
  ...
  describe('#Submit Campaign tests', () => {
    it('Test submit campaign returns correct data after created', async function () {
      ...
      for (let id = 0; id <= 2; id++) {
        ...
        let tx = await submitNewCampaign(
          kyberDao,
          id,
          currentBlock + 2 * id + 5,
          currentBlock + 2 * id + 5 + minCampPeriod,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, 2, 3, 4],
        ...
        Helper.assertEqual(
          blockToTimestamp(currentBlock + 2 * id + 5),
          data.startTimestamp,
          'start time is incorrect'
        );
        Helper.assertEqual(
          blockToTimestamp(currentBlock + 2 * id + 5 + minCampPeriod),
          data.endTimestamp,
    ...
    it('Test submit new campaign for next epoch, data changes as expected', async function () {
      ...
      Helper.assertEqual(
        blockToTimestamp(startBlock + 1 + minCampPeriod),
        data.endTimestamp,
        'end timestamp is incorrect'
      );
    ...
    it('Test submit campaign should revert start or end timestamp is invalid', async function () {
      ...
      await expectRevert(
        kyberDao.submitNewCampaign(
          0,
          currentTimestamp - 1,
          currentTimestamp + minCampPeriod * blockTime,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, 2, 3, 4],
      ...
      await expectRevert(
        kyberDao.submitNewCampaign(
          0,
          daoStartTime + epochPeriod * blockTime,
          daoStartTime + (epochPeriod + minCampPeriod) * blockTime,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, 2, 3, 4],
      ...
      await expectRevert(
        kyberDao.submitNewCampaign(
          0,
          daoStartTime + 10 * epochPeriod * blockTime,
          daoStartTime + (10 * epochPeriod + minCampPeriod) * blockTime,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, 2, 3, 4],
      ...
      await expectRevert(
        kyberDao.submitNewCampaign(
          0,
          daoStartTime - 1,
          daoStartTime + minCampPeriod * blockTime,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, 2, 3, 4],
      ...
      await expectRevert(
        kyberDao.submitNewCampaign(
          0,
          daoStartTime,
          daoStartTime + minCampPeriod * blockTime - 2,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, 2, 3, 4],
    ...
    it('Test submit campaign with startTimestamp is beginning of an epoch', async () => {
      ...
      await kyberDao.submitNewCampaign(
        0,
        startEpoch2Timestamp,
        startEpoch2Timestamp + minCampPeriod * blockTime,
        minPercentageInPrecision,
        cInPrecision,
        tInPrecision,
        [1, 2, 3, 4],
      ...
      await kyberDao.submitNewCampaign(
        0,
        startEpoch2Timestamp,
        startEpoch2Timestamp + blocksToSeconds(epochPeriod) - 1,
        minPercentageInPrecision,
        cInPrecision,
        tInPrecision,
        [1, 2, 3, 4],
    ...
    it('Test submit campaign with endTimestamp is end timestamp of an epoch', async () => {
      ...
      await kyberDao.submitNewCampaign(
        0,
        endEpoch1Timestamp - minCampPeriod * blockTime,
        endEpoch1Timestamp,
        minPercentageInPrecision,
        cInPrecision,
        tInPrecision,
      ...
      await kyberDao.submitNewCampaign(
        0,
        endEpoch1Timestamp - minCampPeriod * blockTime - 10,
        endEpoch1Timestamp,
        minPercentageInPrecision,
        cInPrecision,
        tInPrecision,
    ...
    it('Test submit campaign should revert option value is invalid', async function () {
      ...
      await expectRevert(
        submitNewCampaign(
          ...
          2,
          currentBlock + 15,
          currentBlock + 15 + minCampPeriod,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, getDataFromRebateAndReward(100, 10001 - 100), 2, 3],
          '0x',
          {from: daoOperator}
        ),
      ...
      await expectRevert(
        submitNewCampaign(
          ...
          2,
          currentBlock + 17,
          currentBlock + 17 + minCampPeriod,
          minPercentageInPrecision,
          cInPrecision,
          tInPrecision,
          [1, 2, getDataFromRebateAndReward(20, 10000)],
          '0x',
          {from: daoOperator}
        ),
        "validateParams: rebate + reward can't be bigger than BPS"
      ...
      await submitNewCampaign(
        ...
        2,
        currentBlock + 19,
        currentBlock + 19 + minCampPeriod,
        minPercentageInPrecision,
        cInPrecision,
        tInPrecision,
        [1, getDataFromRebateAndReward(2500, 2500), 2, 3],
        '0x',
        {from: daoOperator}
      );
    });
    ...
    it('Test submit campaign should revert formula params are invalid', async function () {
      ...
      await submitNewCampaign(
        ...
        currentBlock + 5,
        currentBlock + 5 + minCampPeriod,
        precisionUnits.sub(new BN(100)),
        new BN(2).pow(new BN(128)).sub(new BN(1)),
        tInPrecision,
        [1, 2, 3],
        '0x',
        {from: daoOperator}
  ...
  describe('#Cancel Campaign tests', () => {
    it('Test cancel campaign should revert campaign is not existed', async function () {
      ...
      await expectRevert(kyberDao.cancelCampaign(1, {from: daoOperator}), "cancelCampaign: campaignID doesn't exist");
      await updateCurrentBlockAndTimestamp();
      await submitNewCampaign(
        kyberDao,
        0,
      ...
      await expectRevert(kyberDao.cancelCampaign(2, {from: daoOperator}), "cancelCampaign: campaignID doesn't exist");
      await kyberDao.cancelCampaign(1, {from: daoOperator});
    });
    ...
    it('Test cancel campaign should revert sender is not daoOperator', async function () {
      ...
      await expectRevert(kyberDao.cancelCampaign(1, {from: mike}), 'only daoOperator');
      await kyberDao.cancelCampaign(1, {from: daoOperator});
    });
    ...
    it('Test cancel campaign should revert camp already started or ended', async function () {
      ...
      await expectRevert(kyberDao.cancelCampaign(1, {from: daoOperator}), 'cancelCampaign: campaign already started');
    ...
    it('Test cancel campaign should update correct data after cancelled', async function () {
      ...
      for (let id = 0; id < 2; id++) {
        ...
        Helper.assertEqual(
          await kyberDao.numberCampaigns(),
          campCounts,
          'number campaigns have been created is incorrect'
        );
        ...
        Helper.assertEqual(
          false,
          await kyberDao.campaignExists(campCounts - 3),
          "camp shouldn't be existed after cancel"
        );
    ...
    it('Test cancel campaign correctly for network fee camp', async function () {
      ...
      Helper.assertEqual(
        campData.startTimestamp,
        blockToTimestamp(currentBlock + 15),
        'camp details should be correct'
      );
      Helper.assertEqual(
        campData.endTimestamp,
        blockToTimestamp(currentBlock + 15 + minCampPeriod),
        'camp details should be correct'
      );
      Helper.assertEqual(campData.totalKNCSupply, await kncToken.totalSupply(), 'camp details should be correct');
      Helper.assertEqual(
        campData.minPercentageInPrecision,
        minPercentageInPrecision,
        'camp details should be correct'
      );
      ...
      Helper.assertEqual(
        campData.startTimestamp,
        blockToTimestamp(currentBlock + 20),
        'camp details should be correct'
      );
      Helper.assertEqual(
        campData.endTimestamp,
        blockToTimestamp(currentBlock + 20 + minCampPeriod),
        'camp details should be correct'
      );
      Helper.assertEqual(campData.totalKNCSupply, await kncToken.totalSupply(), 'camp details should be correct');
      Helper.assertEqual(
  ...
  describe('#Vote tests', () => {
    ...
    it('Test vote should revert camp is not existed', async function () {
      ...
      await expectRevert(kyberDao.vote(1, 1, {from: mike}), "vote: campaign doesn't exist");
      ...
      await expectRevert(kyberDao.vote(2, 1, {from: mike}), "vote: campaign doesn't exist");
    ...
    it('Test vote should revert camp has not started or already ended', async function () {
      ...
      await expectRevert(kyberDao.vote(1, 1, {from: mike}), 'vote: campaign not started');
      ...
      // camp alread ended
      await expectRevert(kyberDao.vote(1, 1, {from: mike}), 'vote: campaign already ended');
    });
    ...
    it('Test vote should revert when voted option is invalid', async function () {
      ...
      // can not vote for 0
      await expectRevert(kyberDao.vote(1, 0, {from: mike}), 'vote: option is 0');

      // can not vote for option that is bigger than range
      await expectRevert(kyberDao.vote(1, 3, {from: mike}), 'vote: option is not in range');
  ...
  describe('#Get staker percentage tests', () => {
    ...
    it('Test get staker reward percentage - with delegation', async function () {
      ...
      // should return all 0
      await verifyPastEpochtStakerRewardPercentages([mike, poolMaster, poolMaster2, victor, loi], [0, 0, 0, 0, 0], 1);
    ...
    it('Test get staker percentage after few epochs', async function () {
      ...
      await verifyCurrentEpochStakerRewardPercentages([mike, poolMaster, poolMaster2, victor, loi], [0, 0, 0, 0, 0]);
    });
    ...
    it('Test get staker percentage for current and future epochs', async function () {
      ...
      await verifyPastEpochtStakerRewardPercentages([mike, victor], [0, 0], 2);
      await verifyPastEpochtStakerRewardPercentages([mike, victor], [0, 0], 100);

      // get staker percentage for current epoch, return 0
      await verifyPastEpochtStakerRewardPercentages([mike, victor], [0, 0], 1);
      ...
      await verifyCurrentEpochStakerRewardPercentages([mike, victor], [mikePercentage, victorPercentage]);

      // delay to epoch 2
      await Helper.mineNewBlockAt(blocksToSeconds(epochPeriod) + daoStartTime);
      // verify past epoch data
      await verifyPastEpochtStakerRewardPercentages([mike, victor], [mikePercentage, victorPercentage], 1);
      ...
      await verifyCurrentEpochStakerRewardPercentages([mike, victor], [precisionUnits, 0]);
    ...
    it('Test get reward percentage after many epochs', async function () {
      ...
      await verifyPastEpochtStakerRewardPercentages([mike, poolMaster, poolMaster2, victor, loi], [0, 0, 0, 0, 0], 3);
    });
    ...
    it('Test get staker reward percentage when no votes', async function () {
      ...
      Helper.assertEqual(
        0,
        await kyberDao.getPastEpochRewardPercentageInPrecision(mike, 0),
        'reward percentage is wrong'
      );
    });
  ...
  describe('#Get Network Fee Data tests', () => {
    ...
    it('Test get network fee returns correct latest data, no campaigns', async function () {
      ...
      Helper.assertEqual(defaultNetworkFee, feeData.feeInBps, 'network fee default is wrong');
      Helper.assertEqual(
        blocksToSeconds(epochPeriod) + daoStartTime - 1,
        feeData.expiryTimestamp,
        'expiry timestamp is wrong'
      );
      ...
      Helper.assertEqual(32, feeData.feeInBps, 'network fee default is wrong');
      Helper.assertEqual(
        blocksToSeconds(4 * epochPeriod) + daoStartTime - 1,
        feeData.expiryTimestamp,
        'expiry timestamp is wrong'
      );
    ...
    it('Test get network fee returns correct latest data, has camp but no fee campaign', async function () {
      ...
      Helper.assertEqual(
        blocksToSeconds(epochPeriod * 2) + daoStartTime - 1,
        data[1],
        'expiry timestamp is wrong for epoch 1'
      );
    });
  ...
  describe('#Get BRR Data tests', () => {
    it('Test get brr data returns correct default data for epoch 0', async function () {
      ...
      await checkLatestBrrData(reward, rebate, 10000 - rebate - reward, 0, daoStartTime - 1);
    ...
    it('Test get brr data returns correct latest data, no campaigns', async function () {
      ...
      await checkLatestBrrData(
        ...
        10000 - rebate - reward,
        4,
        blocksToSeconds(epochPeriod * 4) + daoStartTime - 1
      );
    ...
    it('Test get brr returns correct latest data, has brr camp but no winning option', async function () {
      ...
      await checkLatestBrrData(
        ...
        10000 - rebate - reward,
        3,
        blocksToSeconds(epochPeriod * 3) + daoStartTime - 1
      );
  ...
  describe('#Constructor tests', () => {
    it('Test correct data is set after deployment', async function () {
      ...
      Helper.assertEqual(
        await kyberDao.minCampaignDurationInSeconds(),
        blocksToSeconds(minCampPeriod),
        'min camp period is wrong'
      );
      Helper.assertEqual(
        (await kyberDao.getLatestNetworkFeeData()).feeInBps,
        defaultNetworkFee,
        'default network fee is wrong'
      );
      ...
      Helper.assertEqual(
        await kyberDao.epochPeriodInSeconds(),
        await stakingContract.epochPeriodInSeconds(),
        'epochPeriodInSeconds is wrong'
      );
      Helper.assertEqual(
        await kyberDao.firstEpochStartTimestamp(),
        await stakingContract.firstEpochStartTimestamp(),
        'firstEpochStartTimestamp is wrong'
      );
    ...
    it('Test constructor should revert invalid arguments', async function () {
      ...
      await expectRevert(
        KyberDaoContract.new(
          0,
          blockToTimestamp(currentBlock + 50),
          kncToken.address,
          minCampPeriod,
          defaultNetworkFee,
          defaultRewardBps,
  ...
  describe('#Helper Function tests', () => {
    ...
    it('Test getDataFromRewardAndRebateWithValidation should revert total amount > bps (10000)', async function () {
      ...
      let reward = 10001;
      let rebate = 0;
      await expectRevert(kyberDao.getDataFromRewardAndRebateWithValidation(reward, rebate), 'reward plus rebate high');
