...
const BadToken = artifacts.require('TokenNotReturn.sol');
...
contract('KyberFeeHandler', function (accounts) {
  ...
  describe('should test events declared in feeHandler', async () => {
    ...
    it('FeeDistributed (no BRR)', async () => {
      ...
      let txResult = await feeHandler.handleFees(ethAddress, [], [], platformWallet, oneEth, zeroBN, {
        from: kyberNetwork,
        value: oneEth,
      });
      ...
      Helper.assertEqual(
        txResult.logs[0].args.rebatePercentBpsPerWallet.length,
        zeroBN,
        'unexpected rebate percent bps length'
      );
    });
    ...
    it('FeeDistributed (with BRR)', async () => {
      ...
      for (let i = 0; i < txResult.logs[1].args.rebateWallets.length; i++) {
        ...
        Helper.assertEqual(
          txResult.logs[1].args.rebatePercentBpsPerWallet[i],
    ...
    it('RewardPaid', async () => {
      ...
      mockKyberDao = await MockKyberDao.new(rewardInBPS, rebateInBPS, currentEpoch.add(new BN(1)), expiryTimestamp);
      await feeHandler.setDaoContract(mockKyberDao.address, {from: daoSetter});
  ...
  describe('test getBRR and updateBRRData functions', async () => {
    ...
    it('should revert if burnBps causes overflow', async () => {
      ...
      await expectRevert.unspecified(feeHandler.getBRR());
    });
    ...
    it('should revert if bad BRR values are returned', async () => {
      ...
      await expectRevert(feeHandler.getBRR(), 'Bad BRR values');
    });
    ...
    it('should revert if expiry timestamp >= 2 ** 64', async () => {
      ...
      await expectRevert(feeHandler.getBRR(), 'expiry timestamp overflow');
    ...
    it('should revert if epoch >= 2 ** 32', async () => {
      ...
      await expectRevert(feeHandler.getBRR(), 'epoch overflow');
  ...
  describe('test permissions: onlyKyberDao, onlyKyberNetwork, only dao setter', async () => {
    ...
    it('reverts if non-KyberDao setter tries to set KyberDao contract', async () => {
      await expectRevert(feeHandler.setDaoContract(mockKyberDao.address, {from: user}), 'only daoSetter');
    });
  });
  ...
  describe('test handle fees and claiming rebate / reward / fee', async () => {
    ...
    describe('reserve rebate', async () => {
      ...
      it('reverts if reserve has no rebate to claim', async () => {
        await expectRevert(feeHandler.claimReserveRebate(user), 'no rebate to claim');
      });
      ...
      it('reverts if rebate wallet is non-payable contract', async () => {
        ...
        expectedRebates = await callHandleFeeAndVerifyValues(
          ...
          currentRewardBps,
          currentEpoch,
          [rebateWallet.address],
          rebateBpsPerWallet
        );

        await expectRevert(feeHandler.claimReserveRebate(rebateWallet.address), 'rebate transfer failed');
      });
      ...
      it('reverts if totalPayoutBalance < amount', async () => {
        ...
        let feeBRRWei = oneEth;
        let sendVal = platformFeeWei.add(feeBRRWei);
        feeHandler = await BadFeeHandler.new(
          daoSetter,
          proxy.address,
          kyberNetwork,
          knc.address,
        ...
        await expectRevert.assertion(feeHandler.claimReserveRebate(rebateWallets[0]));
      });
    });
    ...
    describe('staking rewards', async () => {
      beforeEach('init contracts before each test', async () => {
        ...
        mockKyberDao = await MockKyberDao.new(rewardInBPS, rebateInBPS, currentEpoch, expiryTimestamp);
        await feeHandler.setDaoContract(mockKyberDao.address, {from: daoSetter});
      });
      ...
      it('reverts if total percentage of all stakers more than 100%', async () => {
        ...
        await expectRevert.assertion(feeHandler.claimStakerReward(accounts[4], currentEpoch));
      });
      ...
      it('reverts if totalPayoutBalance < stakerAmt', async () => {
        ...
        await mockKyberDao.advanceEpoch();
        await expectRevert.unspecified(feeHandler.claimStakerReward(user, currentEpoch, {from: user}));
      });
    ...
    describe('platform fee', async () => {
      ...
      it('reverts if platformWallet has no fee to claim', async () => {
        await expectRevert(feeHandler.claimPlatformFee(user), 'no fee to claim');
      });
      ...
      it('reverts if totalPayoutBalance < platformFeeWei', async () => {
        ...
        await feeHandler.setTotalPayoutBalance(zeroBN);
        await expectRevert.assertion(feeHandler.claimPlatformFee(platformWallet));
      });
    });
    ...
    describe('burning', async () => {
      ...
      it('burn KNC test correct burn_wait_interval for next burn', async () => {
        ...
        while (nextBurnBlock > currentBlock) {
          await expectRevert(feeHandler.burnKnc(), 'wait more blocks to burn');
      ...
      it('reverts if contract has insufficient ETH for burning', async () => {
        ...
        await expectRevert.assertion(feeHandler.burnKnc());
      });
      ...
      it('reverts if ETH-KNC > MAX_RATE', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'ethToKnc rate out of bounds');
      });
      ...
      it('reverts if ETH-KNC = 0', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'ethToKnc rate is 0');
      });
      ...
      it('reverts no sanity rate contract', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'no sanity rate contract');
      });
      ...
      it('reverts sanity rate 0', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'sanity rate is 0');
      });
      ...
      it('reverts sanity rate > MAX_RATE', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'sanity rate out of bounds');
      });
      ...
      it('reverts sanity rate and ethToKnc rate diff > MAX_DIFF of 10%', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'kyberNetwork eth to knc rate too low');
      });
      ...
      it('reverts only none contract can call burn', async () => {
        ...
        await expectRevert(contract.callBurnKnc(), 'only non-contract');
      });
      ...
      it('reverts if sanity rate is 0', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'sanity rate is 0x0, burning is blocked');
      });
      ...
      it('reverts if malicious KNC token is used, and burning fails', async () => {
        ...
        await expectRevert(feeHandler.burnKnc(), 'knc burn failed');
      });
      ...
      it('reverts if kyberDao is not set', async () => {
        ...
        await expectRevert(feeHandler.makeEpochRewardBurnable(currentEpoch), 'kyberDao not set');
      });
      ...
      it('reverts if kyberDao prevents burning of reward', async () => {
        ...
        await expectRevert(feeHandler.makeEpochRewardBurnable(currentEpoch), 'should not burn reward');
      });

      it('reverts if no reward to burn', async () => {
        await mockKyberDao.setShouldBurnRewardTrue(currentEpoch);
        await expectRevert(feeHandler.makeEpochRewardBurnable(currentEpoch), 'reward is 0');
      });
      ...
      it('reverts if totalPayoutBalance < rewardAmount', async () => {
        ...
        await expectRevert(feeHandler.makeEpochRewardBurnable(currentEpoch), 'total reward less than epoch reward');
      });
    });
    ...
    describe('test re entrance for claim fee functions is blocked', async () => {
      ...
      it('revert if call trade during claim platform fee', async () => {
        ...
        await expectRevert(feeHandler.claimPlatformFee(claimer.address), 'platform fee transfer failed');
      ...
      it('revert if call trade during claim rebate', async () => {
        ...
        await expectRevert(feeHandler.claimReserveRebate(claimer.address), 'rebate transfer failed');
        // when no re entrance, claim fee succeeds
        await claimer.setReentrancy(false);
  ...
  it('test upgradeable - update network address', async () => {
    ...
    tempFeeHandler = await FeeHandler.new(
      daoSetter,
      tempProxy.address,
      kyberNetwork,
      knc.address,
    ...
    await expectRevert(tempFeeHandler.setNetworkContract(accounts[0]), 'only daoOperator');
    await expectRevert(tempFeeHandler.setNetworkContract(zeroAddress, {from: daoOperator}), 'kyberNetwork 0');
  ...
  describe('Update Network Proxy', async () => {
    ...
    it('Test should revert not daoOperator', async () => {
      await expectRevert(feeHandler.setKyberProxy(accounts[0], {from: accounts[0]}), 'only daoOperator');
    });
    ...
    it('Test burn KNC after updating proxy', async () => {
      ...
      let tempFeeHandler = await FeeHandler.new(
        daoSetter,
        tempProxy.address,
        tempNetwork,
        knc.address,
...
async function callHandleFeeAndVerifyValues(
  sendValWei,
  platformWallet,
  platFeeWei,
  rebateBps,
  ...
  // handle fees
  await feeHandler.handleFees(ethAddress, rebateWalletArr, rebateBpsArr, platformWallet, platFeeWei, feeAmountBRR, {
    from: kyberNetwork,
    value: sendValWei,
  });
