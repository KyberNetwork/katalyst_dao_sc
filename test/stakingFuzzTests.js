...
contract('KyberStaking simulator', async (accounts) => {
  before('one time init: Stakers, KyberStaking, KNC token', async () => {
    ...
    kyberStaking = await MockKyberStaking.new(kncToken.address, epochPeriod, firstBlockTimestamp + 1000, daoOperator);
  });
  ...
  it(`fuzz tests kyberStaking contract with ${NUM_RUNS} loops`, async () => {
    await StakeSimulator.doFuzzStakeTests(kyberStaking, NUM_RUNS, kncToken, stakers, epochPeriod);
  });
});
