...
const {BPS, precisionUnits, ethDecimals, ethAddress, zeroAddress} = require('./helper.js');
...
contract('KyberFeeHandlerWrapper', function (accounts) {
  ...
  beforeEach('setup feeHandlers and wrapper', async () => {
    // setup feeHandlers
    ethFeeHandler = await FeeHandler.new(
      daoSetter,
      proxy.address,
      kyberNetwork,
      knc.address,
    ...
    daiFeeHandler = await FeeTokenHandler.new(
    ...
    eursFeeHandler = await FeeTokenHandler.new(
      mockKyberDao.address,
      proxy.address,
      kyberNetwork,
      eurs.address,
    ...
    usdcFeeHandler = await FeeTokenHandler.new(
      mockKyberDao.address,
      proxy.address,
      kyberNetwork,
      usdc.address,
  ...
  describe('test handle fees and claiming rebate / reward / fee using wrapper', async () => {
    ...
    it('should not revert if startTokenIndex > endTokenIndex', async () => {
      ...
      await feeWrapper.claimStakerReward(
        staker,
        previousEpoch,
        startTokenIndex,
        endTokenIndex,
        startFeeHandlerIndex,
        endFeeHandlerIndex
      );
      ...
      await feeWrapper.claimPlatformFee(
        platformWallet,
        startTokenIndex,
        endTokenIndex,
        startFeeHandlerIndex,
      ...
      for (const rebateWallet of rebateWallets) {
        ...
        await feeWrapper.claimReserveRebate(
          rebateWallet,
          startTokenIndex,
          endTokenIndex,
          startFeeHandlerIndex,
    ...
    it('should not revert if staker manually claimed from at least 1 feeHandler', async () => {
      ...
      assertSameBalances(
        initialBalancesExpectNoChange,
        actualBalancesExpectNoChange,
        "balances didn't remain the same"
      );
  ...
  describe('test with multiple tokenFeeHandlers for a token', async () => {
    ...
    before('setup tempFeeHandlers, get token index in feeWrapper', async () => {
      ...
      tempUsdcFeeHandler1 = await FeeTokenHandler.new(
        mockKyberDao.address,
        proxy.address,
        kyberNetwork,
        usdc.address,
      ...
      tempUsdcFeeHandler2 = await FeeTokenHandler.new(
        mockKyberDao.address,
        proxy.address,
        kyberNetwork,
        usdc.address,
    ...
    it("should claim from only 1 feeHandler if epochs don't overlap", async () => {
      ...
      Helper.assertEqual(
        await tempUsdcFeeHandler2.hasClaimedReward(staker, selectedEpoch),
        false,
        'staker should not have claimed reward'
      );
      Helper.assertEqual(
        await tempUsdcFeeHandler1.hasClaimedReward(staker, selectedEpoch),
        false,
        'staker should not have claimed reward'
      );
      Helper.assertEqual(
        await usdcFeeHandler.hasClaimedReward(staker, selectedEpoch),
        false,
        'staker should not have claimed reward'
      );
      ...
      Helper.assertEqual(
        await tempUsdcFeeHandler2.hasClaimedReward(staker, selectedEpoch),
        true,
        'staker should have claimed reward'
      );
      Helper.assertEqual(
        await tempUsdcFeeHandler1.hasClaimedReward(staker, selectedEpoch),
        false,
      ...
      for (const rebateWallet of rebateWallets) {
        for (const feeHandler of tempFeeHandlers) {
          Helper.assertGreater(
            await feeHandler.rebatePerWallet(rebateWallet),
            new BN(1),
            'feeHandler has no reserve rebate'
          );
        }
        ...
        for (const feeHandler of tempFeeHandlers) {
          Helper.assertEqual(
            await feeHandler.rebatePerWallet(rebateWallet),
            new BN(1),
            'did not claim reserve rebate from feeHandler'
          );
        }
    ...
    it('should claim platform fees from all feeHandlers', async () => {
      ...
      for (const feeHandler of tempFeeHandlers) {
        Helper.assertGreater(
          await feeHandler.feePerPlatformWallet(platformWallet),
          new BN(1),
          'feeHandler has no reserve rebate'
        );
      }
