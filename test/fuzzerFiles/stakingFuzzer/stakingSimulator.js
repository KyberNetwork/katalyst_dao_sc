...
module.exports.doFuzzStakeTests = async function (kyberStaking, NUM_RUNS, kncToken, stakers, epochPeriod) {
  let result;
  let validity;
...
async function executeDeposit(kyberStaking, result, epochPeriod) {
  ...
  if (result.isValid) {
    ...
    if (result.revertMsg != '') {
      await expectRevert(kyberStaking.deposit(result.amount, {from: result.staker}), result.revertMsg);
    } else {
      await expectRevert.unspecified(kyberStaking.deposit(result.amount, {from: result.staker}));
    }
  }
}
...
async function executeDelegate(kyberStaking, result, epochPeriod) {
  ...
  if (result.dAddress == zeroAddress) {
    await expectRevert(kyberStaking.delegate(result.dAddress, {from: result.staker}), 'delegate: representative 0');
  } else {
...
async function executeWithdraw(kyberStaking, result, epochPeriod) {
  ...
  if (result.isValid) {
    ...
    await expectRevert(kyberStaking.withdraw(result.amount, {from: result.staker}), result.revertMsg);
  }
}
...
async function executeNoAction(epochPeriod) {
  ...
  let currentBlockTime = await Helper.getCurrentBlockTime();
  await Helper.mineNewBlockAt(currentBlockTime + Helper.getRandomInt(10, epochPeriod.toNumber()));
}
...
async function getState(kyberStaking, result, oldRepAddress) {
  ...
  res.oldRepAddress =
    oldRepAddress == undefined ? await kyberStaking.getRepresentative(result.staker, currEpochNum) : oldRepAddress;
