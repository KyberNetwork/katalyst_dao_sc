...
contract('KyberStaking', function (accounts) {
  ...
  describe('#Withdrawal Tests', () => {
    ...
    it('Test withdraw should call KyberDao handleWithdrawal as expected', async function () {
      let dao = await MockKyberDao.new(blocksToSeconds(10), blockToTimestamp(currentBlock + 10));
  ...
  describe('#Revert Tests', () => {
    it('Test constructor should revert with invalid arguments', async function () {
      // knc is 0
      await expectRevert(
        StakingContract.new(zeroAddress, blocksToSeconds(20), blockToTimestamp(currentBlock + 10), kyberDao),
        'ctor: kncToken 0'
      );
      // epoch period is 0
      await expectRevert(
        StakingContract.new(kncToken.address, blocksToSeconds(0), blockToTimestamp(currentBlock + 10), kyberDao),
        'ctor: epoch period is 0'
      );
      // start timestamp is in the past
      await expectRevert(
        StakingContract.new(kncToken.address, blocksToSeconds(20), blockToTimestamp(currentBlock - 1), kyberDao),
        'ctor: start in the past'
      );
      // dao setter is 0
      await expectRevert(
        StakingContract.new(kncToken.address, blocksToSeconds(20), blockToTimestamp(currentBlock + 10), zeroAddress),
        'ctor: kyberDao 0'
      );
      stakingContract = await StakingContract.new(
        kncToken.address,
    ...
    it('Test deposit should revert when not enough balance or allowance', async function () {
      ...
      await kncToken.transfer(victor, mulPrecision(100));
      await kncToken.approve(stakingContract.address, mulPrecision(100), {from: victor});
      await expectRevert(stakingContract.deposit(mulPrecision(200), {from: victor}), 'transfer more then allowed');
      await stakingContract.deposit(mulPrecision(90), {from: victor});
      await stakingContract.deposit(mulPrecision(10), {from: victor});
      ...
      await expectRevert(stakingContract.deposit(mulPrecision(100), {from: mike}), 'transfer more then allowed');

      // approve and deposit more than allowance
      await kncToken.approve(stakingContract.address, mulPrecision(100), {from: mike});
      await expectRevert(stakingContract.deposit(mulPrecision(200), {from: mike}), 'transfer more then allowed');

      // total deposit more than allowances
      await stakingContract.deposit(mulPrecision(50), {from: mike});
      await expectRevert(stakingContract.deposit(mulPrecision(51), {from: mike}), 'transfer more then allowed');
  ...
  describe("Test Withdrawal shouldn't revert", () => {
    it("Test withdraw shouldn't revert when handleWithdrawal in KyberDao reverted", async function () {
      let dao = await MockKyberDaoWithdrawFailed.new(blocksToSeconds(10), blockToTimestamp(currentBlock + 10));
      kyberDao = dao.address;
      await deployStakingContract(10, currentBlock + 10);
  ...
  describe('#Malicious Staking', () => {
    it('Test withdraw should revert, pass checking but not enough knc to withdraw', async function () {
      ...
      await expectRevert(stakingContract.withdraw(mulPrecision(800), {from: victor}), 'sub underflow');
    });
